...
# generate the contents/authors/me.yaml from cv.yaml
about:
	pdm run jinja -d $(CVDIR)/cv.yaml $(J2_DIR)/_me.j2 > $(AUTHOR_DIR)/me.yaml

# generate the static HTML pages for the bibliography
bib: $(VIRTUALENV)
	# Import from BibTexfiles
	pdm run academic import --bibtex $(BIBDIR)/papers.bib content/publications
	pdm run academic import --bibtex $(BIBDIR)/patents.bib content/publications
